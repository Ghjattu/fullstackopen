...
import { BrowserRouter, Link, Route, Routes, useMatch, useNavigate } from "react-router-dom";
...
const Menu = (props) => {
	...
	const anecdote = match ? props.anecdotes.find(anecdote => anecdote.id === Number(match.params.id)) : null
	...
	return (
		<div>
			...
			<Routes>
				<Route path="/" element={<AnecdoteList anecdotes={props.anecdotes}/>}></Route>
				<Route path="/create" element={<CreateNew addNew={props.addNew}/>}></Route>
	...
	return (
		<div>
			...
			<form onSubmit={handleSubmit}>
				...
				<div>
					author
					<input type={author.type} name={author.name} value={author.value} onChange={author.onChange}/>
				</div>
				<div>
					url for more info
					<input type={info.type} name={info.name} value={info.value} onChange={info.onChange}/>
				</div>
